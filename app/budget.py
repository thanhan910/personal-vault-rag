...
MICRO_USD_PER_M_MULTIMODAL_TEXT = 120_000
MICRO_USD_PER_B_MULTIMODAL_PIXELS = 600_000
...
def estimate_multimodal(text_tokens: int, image_pixels: int, video_pixels: int = 0) -> int:
    text_cost = (text_tokens * MICRO_USD_PER_M_MULTIMODAL_TEXT + 999_999) // 1_000_000
    pixel_cost = ((image_pixels + video_pixels) * MICRO_USD_PER_B_MULTIMODAL_PIXELS + 999_999_999) // 1_000_000_000
    return max(1, text_cost + pixel_cost)


def reserve(vault_id: str, model: str, operation: str, units: int, microusd: int, logical_key: str) -> Reservation:
    stamp = now()
    ...
    with connect() as db, transaction(db, immediate=True):
        ...
        spent = db.execute(
            """SELECT COALESCE(SUM(COALESCE(actual_microusd,reserved_microusd)),0) amount
               FROM usage_ledger WHERE vault_id=? AND created_at>=? AND state IN ('reserved','complete','uncertain')""",
        ...
        attempt_key = f"{logical_key}:{uuid.uuid4().hex}"
        db.execute(
            """INSERT INTO usage_ledger(id,vault_id,provider,model,operation,estimated_units,reserved_microusd,state,created_at,request_key,logical_key)
               VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
            (reservation_id, vault_id, "voyage", model, operation, units, microusd, "reserved", stamp, attempt_key, logical_key),
        )
...
def mark_uncertain(reservation_id: str, reason: str) -> None:
    with connect() as db:
        db.execute(
            "UPDATE usage_ledger SET state='uncertain',uncertainty_reason=?,completed_at=? WHERE id=?",
            (reason[:1000], now(), reservation_id),
        )
