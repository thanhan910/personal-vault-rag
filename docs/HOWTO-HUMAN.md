...
Enter a Voyage key only in **Provider settings** and set an explicit monthly USD allowance. Existing lexical imports are queued for semantic/visual backfill; the readiness cards show completed coverage rather than merely showing that a key exists. The initial import may use text embedding at the provider's current per-token rate and visual embedding by reported text tokens and processed pixels; actual content determines cost. The local ledger reserves conservatively before every attempt and keeps possibly billed failures against the allowance. Start with a small allowance and inspect usage rather than estimating the whole corpus by paid scanning.
...
Ask clients to search before collection-specific answers, fetch adjacent evidence, cite title plus document/version/location IDs, request history explicitly when older revisions matter, compare conflicting sources/revisions, and abstain when evidence is absent. These server instructions assist but cannot guarantee a subscription model's final wording.

Chat attachments are temporary unless you explicitly call save. Remote inspection returns a queued ID to poll. A save remains pending until the Windows connector confirms the exact original bytes were written into the selected destination; an accompanying note becomes a separate sidecar. Saved and indexed are separate states until the destination's next scan. If Windows is offline, the bounded staging copy expires rather than being falsely called saved.
